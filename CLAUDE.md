...
Non-interactive: set both `$VAULT_KEY_FILE` (PEM RSA private key) and `$VAULT_WRAPPED_SECRET` (from `cert-wrap`) to unlock a vault without a human typing anything — see `internal/certkey`.
...
internal/certkey/       concrete Prompter — recovers a secret non-interactively from a cert-wrapped file + private key
internal/cli/           argument parsing + command dispatch (App.Run)
```

`internal/vault` and `internal/assets` depend on nothing internal and do no prompting/printing, which is what makes them directly unit-testable (see `internal/vault/vault_test.go`, `internal/assets/assets_test.go`). `internal/cli` depends on `vault`, `assets`, `certkey` (for the `cert-wrap` command only), and a `Prompter` interface it declares itself; `internal/cli/cli_test.go` drives the full command-dispatch path with a fake `Prompter` and a `t.TempDir()`-backed `vault.Store`, so CLI behavior is tested without a real terminal. `internal/terminal.Prompter` is the only piece that touches an actual TTY (`golang.org/x/term`); `internal/certkey.Prompter` is a non-interactive alternative that satisfies `cli.Prompter` structurally (it doesn't import `internal/cli`) by unwrapping a pre-wrapped secret file with a local RSA private key. `cmd/boxcar/main.go` is the only place that wires everything together, including choosing which `Prompter` to use (`buildPrompter`, based on `$VAULT_KEY_FILE`/`$VAULT_WRAPPED_SECRET`) — it should stay too thin to need tests.
...
- **Sole-vault auto-default only kicks in when nothing else specified the name.** `parseVaultFlag` now returns `(name, explicit, rest, err)`; when `explicit` is false (no `-vault`/`--vault` flag and no `$VAULT_NAME`), `App.Run` calls `App.defaultVaultName`, which uses the sole `vault.*.json` in `Store.Dir` if there's exactly one, else `"dev"`. This only runs for vault-scoped commands (`!noVaultCmd(args[0])`) — `vaults`/`assets`/`cert-gen` ignore `env` entirely. An explicit `-vault`/`$VAULT_NAME` always wins, even when a different single vault exists on disk; zero or multiple vaults on disk always fall back to `"dev"` rather than guessing.
- **`cert-gen` protects only the RSA keypair's *generation*; protecting the resulting `key.pem` at rest is deliberately left to ordinary vault mechanics, not a bespoke feature.** `App.certGen` (`internal/cli/cli.go`) is a thin wrapper over `certkey.GenerateSelfSigned`, writing `outDir/cert.pem` (`0644`) and `outDir/key.pem` (`0600`), creating `outDir` via `os.MkdirAll` if missing (a `noVaultCmd`, so no vault name is resolved for it). The intended pattern — see `TestRun_FullNonInteractiveWorkflow` and `README.md`'s "Full walkthrough" — is `inject -dir` that output folder into its own password-protected vault (e.g. `keyvault`) for safe storage/transport, then `extract -dir` it back out on the target host as a one-time, human-present provisioning step, after which `$VAULT_KEY_FILE` points at the recovered `key.pem` for every subsequent non-interactive run against the *actual* data vault.
...
- **`certkey` wraps the secret via a data key, never RSA-encrypts it directly.** RSA-OAEP has a plaintext-size ceiling set by the key's modulus (~190 bytes for a 2048-bit key with SHA-256 OAEP) that an arbitrary-length passphrase could exceed. `certkey.WrapSecret` generates a fresh random AES-256 data key per wrap, seals the actual secret under it with AES-256-GCM, and only RSA-OAEP-wraps that small fixed-size data key — never remove this indirection to "simplify" to direct RSA encryption, it would silently break on longer secrets. `certkey.Prompter` satisfies `cli.Prompter` structurally (same method signature) without importing `internal/cli`, so there's no import cycle; it ignores both the `prompt` and `confirm` arguments since there's no interactive text to show and no second entry to compare against — correctness comes from RSA-OAEP/AES-GCM authentication in `UnwrapSecret`, not string comparison.
